...
function WriteFormSelectChevron() {
  return (
    <span className="pointer-events-none absolute inset-y-0 right-4 flex items-center text-[color:var(--text-tertiary)]">
...
export function WritePostForm({ mode = "create", initialPost = null }: Props) {
  ...
  const selectClass = "input-field appearance-none py-2 pl-3 pr-11";
    ...
    <form onSubmit={handleSubmit} className="w-full min-w-0 space-y-5">
      ...
      <div className="w-full min-w-0">
        <div className="mb-1 flex flex-wrap items-end justify-between gap-2">
          <label htmlFor="content" className="label-field mb-0">
            본문
          </label>
          <div className="btn-segment-track">
            <button
              type="button"
              onClick={() => setTab("edit")}
              className={`btn-segment ${tab === "edit" ? "btn-segment-active" : "btn-segment-idle"}`}
            >
              작성
            </button>
            <button
              ...
              className={`btn-segment ${tab === "preview" ? "btn-segment-active" : "btn-segment-idle"}`}
            >
              미리보기
            </button>
          </div>
        ...
        <div
          className={`surface-card flex w-full min-w-0 flex-col overflow-hidden border border-[color:var(--border-subtle)] ${
        ...
        >
          {tab === "edit" ? (
            <textarea
              ...
              className="min-h-0 w-full flex-1 resize-y appearance-none border-0 bg-transparent px-4 py-4 font-sans text-base leading-[1.8] tracking-normal text-foreground outline-none [scrollbar-gutter:stable] placeholder:text-[color:var(--text-tertiary)] focus:outline-none focus:ring-0"
            ...
            <div className="min-h-0 w-full flex-1 overflow-y-auto px-4 py-4 [scrollbar-gutter:stable]">
              {content.trim() ? (
                ...
                <p className="m-0 font-sans text-base leading-[1.8] tracking-normal text-[color:var(--text-tertiary)]">
      ...
      <div className="surface-card w-full min-w-0 p-4">
        <label className="flex items-center justify-between gap-3">
          <div>
            <p className="text-sm font-medium text-foreground">Q&A 스레드</p>
          ...
          <input
            ...
            className="h-5 w-5 accent-[color:var(--foreground)]"
          />
        </label>
      </div>
